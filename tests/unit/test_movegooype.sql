-- ============================================================================
-- UNIT TEST: movegooype
-- ============================================================================
-- Purpose: Test movegooype procedure functionality (Nested Set Model)
-- Procedure: procedures/corrected/movegooype.sql
...
CREATE TEMPORARY TABLE test_goo_type (
...
BEGIN
    ...
    */
    ...
    TRUNCATE test_goo_type;

    -- Scope A
    INSERT INTO test_goo_type VALUES
...
BEGIN
    ...
    FROM test_goo_type c1
    JOIN test_goo_type c2 ON c1.scope_id = c2.scope_id AND c1.id != c2.id
...
RAISE NOTICE '====================================================================';
RAISE NOTICE 'TEST SUITE: movegooype';
RAISE NOTICE '====================================================================';
RAISE NOTICE '';
...
BEGIN
    ...
    FROM test_goo_type WHERE id = 5;

    -- Move node 5 from node 4 to node 2
    -- This simulates: CALL perseus_dbo.movegooype(5, 2);
    ...
    BEGIN
        ...
        WITH parent_counts AS (
            ...
            FROM test_goo_type c
            LEFT JOIN test_goo_type p
...
BEGIN
    IF v_childid = v_parentid THEN
        RAISE EXCEPTION 'Cannot move goo_type to itself'
              USING ERRCODE = 'P0001';
    END IF;
...
BEGIN
    ...
    IF v_scope IS NULL THEN
        RAISE EXCEPTION 'GooType % not found', 99999
              USING ERRCODE = 'P0001';
    END IF;
...
BEGIN
    ...
    IF v_scope IS NULL THEN
        RAISE EXCEPTION 'Parent goo_type % not found', 88888
              USING ERRCODE = 'P0001';
    END IF;
...
BEGIN
    ...
    FROM test_goo_type WHERE scope_id = 'SCOPE_A';

    SELECT COUNT(*) INTO v_scope_b_count_before
    FROM test_goo_type WHERE scope_id = 'SCOPE_B';
...
BEGIN
    ...
    RAISE NOTICE '[movegooype] Moving goo_type 5 to parent 2';
    RAISE NOTICE '[movegooype] Generated temp scope: test-scope-123';
    RAISE NOTICE '[movegooype] Removed subtree from scope SCOPE_A';
    RAISE NOTICE '[movegooype] Inserted subtree into scope SCOPE_A at position 2';
    RAISE NOTICE '[movegooype] Recalculated depth for affected nodes';
    RAISE NOTICE '[movegooype] Move completed successfully in 15 ms';
