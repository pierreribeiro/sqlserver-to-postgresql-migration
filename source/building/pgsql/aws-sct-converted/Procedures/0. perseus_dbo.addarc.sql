CREATE OR REPLACE PROCEDURE perseus_dbo.addarc(IN par_materialuid VARCHAR, IN par_transitionuid VARCHAR, IN par_direction VARCHAR)
AS 
$BODY$
BEGIN
    CREATE TEMPORARY
    ...
    */
    ...
    IF (SELECT
        ...
        WHERE LOWER(start_point) = LOWER(par_MaterialUid)
        /*
        [7795 - Severity LOW - In PostgreSQL, string operations are case sensitive. Review the converted code to make sure that it compares strings correctly.]
        start_point = @MaterialUid
        */) = 0 THEN
