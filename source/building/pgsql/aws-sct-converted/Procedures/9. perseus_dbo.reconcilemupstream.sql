        ...
        SELECT DISTINCT
            ...
            WHERE EXISTS (SELECT
                ...
                */) AND NOT EXISTS (SELECT
                ...
                WHERE LOWER(dl1.uid) = LOWER(mu.start_point)
                /*
                [7795 - Severity LOW - In PostgreSQL, string operations are case sensitive. Review the converted code to make sure that it compares strings correctly.]
                dl1.uid = mu.start_point
                */) AND LOWER(start_point) != LOWER('n/a')
            /*
            [7795 - Severity LOW - In PostgreSQL, string operations are case sensitive. Review the converted code to make sure that it compares strings correctly.]
            start_point != 'n/a'
            */;
        ...
        IF var_dirty_count > 0 THEN
            DELETE FROM perseus_dbo.m_upstream_dirty_leaves
                WHERE EXISTS (SELECT
                    ...
                    WHERE LOWER(d.uid) = LOWER(m_upstream_dirty_leaves.material_uid)
                    /*
                    [7795 - Severity LOW - In PostgreSQL, string operations are case sensitive. Review the converted code to make sure that it compares strings correctly.]
                    d.uid = m_upstream_dirty_leaves.material_uid
                    */);
