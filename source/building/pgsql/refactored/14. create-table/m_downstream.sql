...
-- Description: Cached downstream lineage graph for material tracking
-- ============================================================================
-- Migration Info:
--   Original: source/original/sqlserver/8. create-table/perseus.dbo.m_downstream.sql
--   AWS SCT: source/original/pgsql-aws-sct-converted/14. create-table/44. perseus.m_downstream.sql
--   Quality Score: 9.0/10
--   Analyst: Claude (Database Expert Agent)
--   Date: 2026-01-26
-- ============================================================================
-- Dependencies:
--   Tables: None (Tier 0 - populated by mcgetdownstream stored procedure)
--   Referenced by: mcgetdownstream, mcgetdownstreambylist functions
-- ============================================================================
...
--   [✓] V. Naming & Scoping - snake_case, schema-qualified (perseus.m_downstream)
...
--   - Populated by mcgetdownstream stored procedure
-- ============================================================================
-- Change Log:
...
-- Index for reverse lookups (finding all descendants of a given endpoint)
CREATE INDEX idx_m_downstream_end_level ON perseus.m_downstream(end_point, level);
...
COMMENT ON TABLE perseus.m_downstream IS
'CRITICAL PERFORMANCE TABLE: Cached downstream lineage graph for material tracking.
Stores materialized paths from start_point (ancestor) to end_point (descendant).
Enables fast descendant queries without recursive CTEs (100,000+ rows).
Populated by mcgetdownstream stored procedure.
IMPORTANT: Composite PK (start_point, end_point) prevents duplicates.
Updated: 2026-01-26 | Owner: Perseus DBA Team';

COMMENT ON COLUMN perseus.m_downstream.start_point IS
'Starting material UID (ancestor) - part of composite PK';

COMMENT ON COLUMN perseus.m_downstream.end_point IS
'Ending material UID (descendant) - part of composite PK';

COMMENT ON COLUMN perseus.m_downstream.path IS
'Serialized lineage path from start_point to end_point (VARCHAR 4000)';

COMMENT ON COLUMN perseus.m_downstream.level IS
'Depth level in lineage tree (0 = self, 1 = immediate child, etc.)';
...
-- WHERE table_schema = 'perseus' AND table_name = 'm_downstream'
-- ORDER BY ordinal_position;

-- Verify indexes
-- SELECT indexname, indexdef
-- FROM pg_indexes
-- WHERE schemaname = 'perseus' AND tablename = 'm_downstream';

-- ============================================================================
-- Usage Example
-- ============================================================================

-- Find all descendants of a material (depth 1-5):
-- SELECT end_point, level, path
-- FROM perseus.m_downstream
-- WHERE start_point = 'M123456'
--   AND level BETWEEN 1 AND 5
-- ORDER BY level;

-- ============================================================================
-- END OF m_downstream TABLE DDL
-- ============================================================================
