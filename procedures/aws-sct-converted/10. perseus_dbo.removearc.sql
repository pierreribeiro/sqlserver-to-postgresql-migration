CREATE OR REPLACE PROCEDURE perseus_dbo.removearc(IN par_materialuid VARCHAR, IN par_transitionuid VARCHAR, IN par_direction VARCHAR)
AS 
$BODY$
/*
*
    ...
    */
    THEN
        DELETE FROM perseus_dbo.material_transition
            ...
            /*
            [7795 - Severity LOW - In PostgreSQL, string operations are case sensitive. Review the converted code to make sure that it compares strings correctly.]
            material_id = @MaterialUid
            */
            AND LOWER(transition_id) = LOWER(par_TransitionUid)
            /*
            [7795 - Severity LOW - In PostgreSQL, string operations are case sensitive. Review the converted code to make sure that it compares strings correctly.]
            transition_id = @TransitionUid
            */;
    ELSE
