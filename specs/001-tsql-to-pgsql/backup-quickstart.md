...
CREATE OR REPLACE VIEW perseus_dbo.translated AS
SELECT
    mt.material_id AS source_material,
    tm.material_id AS destination_material,
    mt.transition_id
...
-- CREATE UNIQUE INDEX ix_translated ON perseus_dbo.translated (...);
```
...
--
-- DEPENDENCIES:
-- - perseus_dbo.material_transition (table)
-- - perseus_dbo.transition_material (table)
--
-- REFRESH STRATEGY:
...
ON perseus_dbo.translated (destination_material, source_material, transition_id);
...
-- GRANTS
-- ===================================================================
GRANT SELECT ON perseus_dbo.translated TO perseus_app;

-- ===================================================================
-- DOCUMENTATION
-- ===================================================================
COMMENT ON MATERIALIZED VIEW perseus_dbo.translated IS
...
INSERT INTO perseus_dbo.transition_material (transition_id, material_id)
VALUES ('TRANS001', 'MAT003'), ('TRANS002', 'MAT004');

-- Refresh materialized view
REFRESH MATERIALIZED VIEW perseus_dbo.translated;
...
DECLARE
    v_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO v_count FROM perseus_dbo.translated;
...
SELECT * FROM perseus_dbo.translated WHERE source_material = 'MATERIAL123';
...
DROP MATERIALIZED VIEW IF EXISTS perseus_dbo.translated CASCADE;
...
**Solution**:
```sql
CREATE UNIQUE INDEX ix_myview_unique ON perseus_dbo.myview (column1, column2);
```
