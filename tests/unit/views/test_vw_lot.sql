-- ===================================================================
-- UNIT TEST: test_vw_lot
-- ===================================================================
-- Purpose: Unit tests for perseus.vw_lot
-- Priority: P2
-- Task: T050 (US1 Phase 3 — Validation)
-- Object: perseus.vw_lot
-- Type: VIEW (standard)
-- Created: 2026-03-08
-- ===================================================================
--
-- Columns: id, uid, name, description, material_type_id, process_id,
--          process_uid, process_name, process_description,
--          process_type_id, run_on, duration, container_id,
--          original_volume, original_mass, triton_task_id,
--          recipe_id, recipe_part_id, manufacturer_id,
--          themis_sample_id, catalog_label, created_on, created_by_id
--
-- Dependencies: perseus.goo, perseus.transition_material,
--               perseus.fatsmurf (all base tables)
--
-- Note: id and uid derive from goo and are always populated.
--       Process columns (process_id, process_uid, etc.) are nullable
--       because materials without an associated process step appear
--       with NULL process columns (LEFT JOIN design).
-- ===================================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_schema = 'perseus'
          AND table_name   = 'vw_lot'
    ) THEN
        INSERT INTO test_results VALUES (
            1,
            'View Existence',
            'FAILED',
            'View perseus.vw_lot not found in information_schema.views',
            0
        );
        RETURN;
    END IF;
...
BEGIN
    ...
    ELSE
        INSERT INTO test_results VALUES (
            ...
            'Column Structure',
            'FAILED',
            'No columns found in information_schema.columns for perseus.vw_lot',
            EXTRACT(MILLISECONDS FROM clock_timestamp() - v_start)::INTEGER
        );
    END IF;
EXCEPTION WHEN OTHERS THEN
...
DECLARE
    ...
    v_row_count BIGINT;
BEGIN
    SELECT COUNT(*) INTO v_row_count FROM perseus.vw_lot;
...
DECLARE
    ...
    v_elapsed INTEGER;
BEGIN
    PERFORM * FROM perseus.vw_lot LIMIT 1000;
...
-- id maps directly from goo.id (the primary key of the base table)
-- and is always populated regardless of whether a process step exists.
-- ---------------------------------------------------------------------
DO $$
DECLARE
    v_start      TIMESTAMPTZ := clock_timestamp();
...
SELECT '=====================================================================' AS separator
UNION ALL SELECT 'UNIT TEST RESULTS: vw_lot'
UNION ALL SELECT '====================================================================='
UNION ALL SELECT '';
