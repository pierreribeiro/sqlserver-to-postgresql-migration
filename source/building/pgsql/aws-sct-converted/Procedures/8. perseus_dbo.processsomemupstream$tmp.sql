CREATE OR REPLACE PROCEDURE perseus_dbo.processsomemupstream$tmp(IN par_dirty_in perseus_dbo.goolist, IN par_clean_in perseus_dbo.goolist)
AS 
$BODY$
CREATE TEMPORARY
/*
...
/* in a previous round */
PERFORM perseus_dbo.goolist$aws$f('"par_dirty_in$aws$tmp"');
INSERT INTO "par_dirty_in$aws$tmp"
SELECT
    *
    FROM UNNEST(par_dirty_in);
PERFORM perseus_dbo.goolist$aws$f('"par_clean_in$aws$tmp"');
INSERT INTO "par_clean_in$aws$tmp"
SELECT
    *
    FROM UNNEST(par_clean_in);
INSERT INTO "var_dirty$aws$tmp"
SELECT DISTINCT
    uid
    FROM "par_dirty_in$aws$tmp" AS d
    WHERE NOT EXISTS (SELECT
        1
        FROM "par_clean_in$aws$tmp" AS c
...
IF var_dirty_count > 0 THEN
    ...
    SELECT
        ...
        JOIN "var_dirty$aws$tmp" AS d
            ON LOWER(d.uid) = LOWER(m_upstream.start_point)
            /*
            [7795 - Severity LOW - In PostgreSQL, string operations are case sensitive. Review the converted code to make sure that it compares strings correctly.]
            d.uid = m_upstream.start_point
            */;
    ...
    IF var_rem_rows > 0 THEN
        ...
        DELETE FROM perseus_dbo.m_upstream
            WHERE start_point IN (SELECT
                ...
                WHERE LOWER(f.start_point) = LOWER(m_upstream.start_point)
                /*
                [7795 - Severity LOW - In PostgreSQL, string operations are case sensitive. Review the converted code to make sure that it compares strings correctly.]
                f.start_point = m_upstream.start_point
                */
                AND LOWER(f.end_point) = LOWER(m_upstream.end_point)
                /*
                [7795 - Severity LOW - In PostgreSQL, string operations are case sensitive. Review the converted code to make sure that it compares strings correctly.]
                f.end_point = m_upstream.end_point
                */
                AND LOWER(f.path) = LOWER(m_upstream.path)
                /*
                [7795 - Severity LOW - In PostgreSQL, string operations are case sensitive. Review the converted code to make sure that it compares strings correctly.]
                f.path = m_upstream.path
                */);
    END IF;
END IF;
/* return the list of processed start_point nodes. */
DROP TABLE IF EXISTS ProcessSomeMUpstream$TMPTBL;
CREATE TEMP TABLE ProcessSomeMUpstream$TMPTBL
AS
SELECT
    *
    FROM "var_dirty$aws$tmp";
