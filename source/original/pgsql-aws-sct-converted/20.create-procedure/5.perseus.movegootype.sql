CREATE OR REPLACE PROCEDURE perseus_dbo.movegootype(IN "@ChildId" INTEGER, IN "@ParentId" INTEGER)
AS 
$BODY$
DECLARE
    ...
    BEGIN
        ...
        */;
        ...
        JOIN (SELECT
            ...
            GROUP BY rw.id) AS d
            ON d.id = rw.id
            WHERE rw_dml.scope_id IN (var_myFormerScope, var_myParentScope) AND rw.ID = rw_dml.ID;
        EXCEPTION
            WHEN OTHERS THEN
