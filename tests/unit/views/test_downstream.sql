-- ===================================================================
-- UNIT TEST: test_downstream
-- ===================================================================
-- Purpose: Test suite for perseus.downstream (recursive CTE view)
-- Priority: P1
-- Task: T049 (US1 Phase 3 — Validation)
-- Object: perseus.downstream
...
--   - perseus.downstream  (view under test)
--
-- Notes:
--   - Tests 3, 5, 6, 7 use sampled row counts to avoid full-graph scan cost.
--   - The view projects: start_point, end_point, path, level.
--     path is declared VARCHAR(4000) (widened from VARCHAR(255) to prevent
--     hard truncation errors in PostgreSQL — see downstream.sql design notes).
--   - CYCLE clause (PostgreSQL 14+) tracks (start_point, child) pairs per
--     recursive path. Not tested here (requires cyclic data); verified by
--     code review in downstream.sql.
--   - Mirror of perseus.upstream: upstream traverses destination→source;
--     downstream traverses source→destination (forward direction).
-- ===================================================================
...
-- ===================================================================
-- Confirms perseus.downstream is registered in information_schema.views.
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE table_schema = 'perseus'
          AND table_name   = 'downstream'
    ) THEN
        v_test_passed   := TRUE;
        v_error_message := NULL;
    ELSE
        v_test_passed   := FALSE;
        v_error_message := 'View perseus.downstream not found in information_schema.views';
    END IF;
...
-- The view uses a column alias list in its DDL
-- (CREATE OR REPLACE VIEW perseus.downstream (start_point, end_point,
-- path, level) AS ...) — these are the authoritative projected names.
-- ===================================================================
DO $$
DECLARE
    v_start_time        TIMESTAMP;
...
-- recursive expansion over all source materials in perseus.translated,
-- which may produce millions of rows on production data.
-- ===================================================================
DO $$
DECLARE
    v_start_time        TIMESTAMP;
...
-- 10 seconds. Threshold guards against catastrophic plan regressions
-- (e.g., missing leading-column index hit on source_material, missing
-- CYCLE guard causing runaway expansion, or hash recursion spill to disk
...
-- start_point from translated.source_material; if that column allows
-- NULL or the forward JOIN degrades, NULL start_points appear.
...
-- the CTE seed or arithmetic error. Tested on a 500-row sample.
-- ===================================================================
DO $$
DECLARE
    v_start_time        TIMESTAMP;
...
-- ===================================================================
-- The anchor term initialises path as CAST('/' AS VARCHAR(4000));
-- each recursive step appends the child node id followed by '/'.
-- Every path in the result must therefore start with '/'.
-- A path that does not start with '/' indicates a concatenation bug
-- or CAST coercion failure. Tested on a 500-row sample.
-- ===================================================================
DO $$
DECLARE
    v_start_time        TIMESTAMP;
...
SELECT '=====================================================================' AS separator
UNION ALL
SELECT 'UNIT TEST RESULTS: perseus.downstream'
UNION ALL
SELECT '====================================================================='
UNION ALL
SELECT '';
