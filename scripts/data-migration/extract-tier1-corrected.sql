-- ============================================================================
-- SQL Server Data Extraction Script - Tier 1 - CORRECTED
-- Perseus Database Migration: 15% Sample Extraction with FK Filtering
-- ============================================================================
-- Purpose: Extract 15% sample from 9 Tier 1 tables (depend only on Tier 0)
-- Prerequisites: extract-tier0-corrected.sql MUST be executed first
-- Output: Temp tables for downstream tier extraction + CSV exports
-- Version: 2.0 (corrected counts, idempotency, error handling, proper order)
-- ============================================================================
...
SET NOCOUNT ON;
PRINT '========================================';
PRINT 'TIER 1 EXTRACTION - Starting (CORRECTED)';
...
IF OBJECT_ID('tempdb..#temp_goo_type') IS NULL
   ...
   OR OBJECT_ID('tempdb..#temp_container_type') IS NULL
   OR OBJECT_ID('tempdb..#temp_container') IS NULL
BEGIN
    PRINT 'ERROR: Critical Tier 0 temp tables not found!';
    PRINT 'You must run extract-tier0-corrected.sql first in this session.';
...
BEGIN TRY
    ...
    IF OBJECT_ID('tempdb..#temp_perseus_user') IS NOT NULL
        DROP TABLE #temp_perseus_user;
...
BEGIN TRY
    ...
    IF OBJECT_ID('tempdb..#temp_property') IS NOT NULL
...
BEGIN TRY
    ...
    IF OBJECT_ID('tempdb..#temp_robot_log_type') IS NOT NULL
        DROP TABLE #temp_robot_log_type;
...
BEGIN TRY
    ...
    IF OBJECT_ID('tempdb..#temp_container_type_position') IS NOT NULL
        DROP TABLE #temp_container_type_position;
...
BEGIN TRY
    ...
    IF OBJECT_ID('tempdb..#temp_goo_type_combine_target') IS NOT NULL
        DROP TABLE #temp_goo_type_combine_target;
...
BEGIN TRY
    ...
    IF OBJECT_ID('tempdb..#temp_container_history') IS NOT NULL
        DROP TABLE #temp_container_history;
...
BEGIN TRY
    ...
    IF OBJECT_ID('tempdb..#temp_workflow') IS NOT NULL
...
BEGIN TRY
    ...
    IF OBJECT_ID('tempdb..#temp_field_map_display_type') IS NOT NULL
        DROP TABLE #temp_field_map_display_type;
...
BEGIN TRY
    ...
    IF OBJECT_ID('tempdb..#temp_field_map_display_type_user') IS NOT NULL
        DROP TABLE #temp_field_map_display_type_user;
...
PRINT '  - perseus_user extracted BEFORE workflow (proper FK order)';
PRINT '  - workflow now includes created_by_id FK filter';
PRINT '';
PRINT 'Next: Run extract-tier2-corrected.sql';
PRINT '========================================';
GO
...
-- 4. Actual row counts may vary from 15% due to FK distribution
